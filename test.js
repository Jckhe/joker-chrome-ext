let elementz = document.querySelectorAll('*');
let body = document.querySelectorAll('body');


//array of random sizes
const randomSizes = ["xx-small","x-small","small","medium","large","x-large","xx-large"];
//array of random font's
const randomFonts = ["Comic Sans MS", "Rockwell", "Copperplate", "Wingdings"]

for (let i = 0; i < elementz.length; i++) {
    elementz[i].style.fontSize = `${randomSizes[randomSizes.length - 2]}`;
    elementz[i].style.fontFamily = `${randomFonts[0]}`;
}

setInterval(() => {
    let randomSizeIndex = Math.floor(Math.random() * randomSizes.length)
    let randomIndex = Math.floor(Math.random() * randomFonts.length)
    for (let i = 0; i < elementz.length; i++) {
        elementz[i].style.fontSize = `${randomSizes[randomSizeIndex]}`;
        elementz[i].style.fontFamily = `${randomFonts[randomIndex]}`;
    }
}, 10000)


//changes color every 3333ms
setInterval(() => {
    let randomIndex = Math.floor(Math.random() * randomColors.length)
    let randomBGColor = randomColors[randomIndex]['name'];
    body[0].style.backgroundColor = `${randomBGColor}`
}, 7210)



//array of random color's
const randomColors = [
    {
      "name": "INDIANRED",
      "hex": "#CD5C5C",
